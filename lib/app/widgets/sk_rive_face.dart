...
class SkRiveFace extends StatefulWidget {
  ...
  //
  // **The faces are not all the same shape, so the box is not square.** The
  // feeling artboards were tightened around their drawings on 23 September
  // 2026, and they came back 360x300 for the girl, 280x240 for the cat and
  // 260x260 for the rabbit -- the rabbit's ears are as tall as her head is
  // wide, and the girl's hair is not. `Fit.contain` fits the drawing inside
  // whatever box it is given, so a square box letterboxes the two wide faces
  // and re-opens exactly the gap the artboards were tightened to close.
  //
  ...
  //
  // **It is a default here rather than something each caller passes**, moved
  // on 24 September 2026 from `FeelingButton.faceAspectRatio`. The picker was
  // the only one of the four screens drawing a head that passed it, so the
  // swap drill's lesson heads got the square box and letterboxed -- and for
  // two readers in three, because the teacher is the next character round the
  // ring and only the rabbit is square. The shape is a fact about the
  // artboards, the artboards are shared, so it belongs beside them rather
  // than on one screen that happens to have noticed.
  //
  ...
  // The shape every head artboard is drawn into. See [aspectRatio] for why it
  // is not 1.0 and why it lives here.
  static const double defaultAspectRatio = 1.15;
