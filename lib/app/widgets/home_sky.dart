...
class HomeSkyColors {
  ...
  // A band of colour where the ranges meet the sky, peaking at `blushAt`.
  // Without it the sky runs from `top` to `bottom` down the whole screen,
  // and a colour meant for the horizon lands behind the panel instead. Day
  // only, for now.
  final Color? blush;

  // A soft mist behind her, in the valley between the ranges, so a pale
  // character has something deeper to stand against. Found on the day sky,
  // where her cream head and the pink horizon were the same lightness and
  // she was reported as washed out, and put on every sky at the user's
  // request. It is the scene's own land colour, a step past the far hills,
  // so it belongs to the picture rather than sitting on it.
  Color get mist => Color.lerp(farHill, land, 0.6)!;

  // How strong the mist is at its heart. Lighter on a deep sky, where she
  // already stands out and a full-strength mist would read as a shadow.
  double get mistStrength => mistLevel ?? (_isDeep ? 0.6 : 0.95);

  // A sky's own mist strength, where the rule above is wrong for it. The
  // midday sky is near-white from the peaks down, and a full-strength mist
  // on it read as a dark blue halo round her.
  final double? mistLevel;

  bool get _isDeep => top.computeLuminance() < 0.2;
  ...
  // Where the blush is strongest, as a share of the screen's height: just
  // under the peaks, in the valley of sky above her hill. Measured off the
  // rendered screen on an ordinary phone.
  static const double blushAt = 0.44;

  // The sky's stops, top to bottom, and where each one sits.
  List<Color> get stops =>
      blush == null ? <Color>[top, bottom] : <Color>[top, blush!, bottom];
  List<double> get stopsAt =>
      blush == null ? <double>[0, 1] : <double>[0, blushAt, 1];

  // The sky's colour at `t`, 0 at the top of the screen and 1 at the foot.
  Color at(double t) {
    final List<Color> c = stops;
    final List<double> s = stopsAt;
    for (int i = 1; i < s.length; i++) {
      if (t <= s[i]) {
        return Color.lerp(c[i - 1], c[i], (t - s[i - 1]) / (s[i] - s[i - 1]))!;
      }
    }
    return c.last;
  }
  ...
  Color get horizon => at(horizonAt);
  ...
  static const Map<(DayPhase, Brightness), HomeSkyColors> _scenes =
    ...
    (DayPhase.morning, Brightness.light): HomeSkyColors(
      ...
      // A step deeper than it was. At `#EBA9C0` the far range and its pines
      // sat at 1.2:1 against the rabbit's peach head, and she was reported
      // as washed out in the middle of them. Lighter pines were considered
      // and would have closed the gap further: her head is already near the
      // lightness of the pale sky, so the only room is below her.
      farHill: Color(0xFFD98AAB),
      land: Color(0xFFC06A92),
      // A soft pink sun, not a cream one. Cream on a yellow-to-pink sky was
      // the sky's own lightness and was reported as washed out; a coral one
      // fixed that and was too strong. Pink sits between the two.
      body: Color(0xFFFFA3A6),
      glow: Color(0xFFFFF6E8),
      star: _starlight,
      onSky: _darkText,
    ),
    ...
    //
    // **The pink is a band at the horizon, not the foot of the screen.** As
    // a plain two-colour sky the pink sat behind the panel and the valley
    // showed lavender; `blush` puts it where the reference has it.
    //
    // **A light baby blue since 26 September 2026, at the user's request.**
    // The sky, the band at the horizon and the ranges all moved from blue
    // and pink to aqua, then the same afternoon from aqua to a softer, paler
    // baby blue, so the whole midday scene is one family. Never green.
    (DayPhase.midday, Brightness.light): HomeSkyColors(
      // White comes in early and stays: baby blue only at the very top,
      // near-white from the peaks down. At the user's request, 26
      // September 2026.
      top: Color(0xFF8FD1EF),
      blush: Color(0xFFF1F8FC),
      bottom: Color(0xFFF8FBFD),
      // Light, because the sky behind it is near-white. At the default
      // 0.95 it read as a dark halo, 26 September 2026.
      mistLevel: 0.45,
      farHill: Color(0xFF7DB0D4),
      land: Color(0xFF4E84B0),
      // A warm gold rather than cream: on the pink the cream sun and its
      // white glow were reported as washed out.
      body: Color(0xFFFFCF5C),
      // Near-white: a yellow glow laid thin over the pink came out beige.
      glow: Color(0xFFFFF8E6),
      star: _starlight,
      onSky: _darkText,
    ),
    // **The afternoon is the day a few hours on**: the same blue, a shade
    // deeper at the top, and the pink at the horizon warmed to peach as the
    // sun starts down. Added 26 September 2026 with the midday sky.
    (DayPhase.afternoon, Brightness.light): HomeSkyColors(
      top: Color(0xFF6CA6DC),
      blush: Color(0xFFF9CDB6),
      bottom: Color(0xFFF6D9CB),
      farHill: Color(0xFF8B8FCC),
      land: Color(0xFF636AB4),
      body: Color(0xFFFFC24A),
    ...
    // **Late night is the night with the lights turned further down**: the
    // sky deeper, the horizon glow nearly gone, the moon paler and quieter
    // and further across the sky. Added 26 September 2026, at the user's
    // request, for the hours after midnight.
    (DayPhase.lateNight, Brightness.light): HomeSkyColors(
      top: Color(0xFF1E2556),
      bottom: Color(0xFF4A568F),
      farHill: Color(0xFF272F66),
      land: Color(0xFF151A3E),
      body: Color(0xFFEEDDB4),
      glow: Color(0xFFC9A477),
      star: _starlight,
      onSky: _lightText,
    ),

    // Dark mode: the deep version.
    // **The dark day skies are dimmed versions of the light ones, not
    // different skies.** Until 26 September 2026 dark midday was a deep
    // navy-teal that read as evening. The user asked whether dark mode
    // needs dark day scenes at all; the answer taken was yes, but lighter:
    // somebody who keeps their phone dark all day chose that, often because
    // a bright screen hurts, and the sky is half the screen. So each one is
    // its light scene's own colours with the lights lowered -- dawn still
    // pink, midday still baby blue going white at the horizon, afternoon
    // still warm at the foot -- dark enough at the top for white words.
    (DayPhase.morning, Brightness.dark): HomeSkyColors(
      top: Color(0xFF3A3262),
      blush: Color(0xFF6A507A),
      bottom: Color(0xFFC9929E),
      farHill: Color(0xFF6A4C74),
      land: Color(0xFF443050),
      body: Color(0xFFE8C98A),
      glow: Color(0xFFD9A77A),
      star: _starlight,
      onSky: _lightText,
    ),
    (DayPhase.midday, Brightness.dark): HomeSkyColors(
      top: Color(0xFF2D5C86),
      blush: Color(0xFF3F6F99),
      bottom: Color(0xFFA9CCE4),
      farHill: Color(0xFF32608A),
      land: Color(0xFF1C3F60),
      // Dimmer than the light sun: it sits behind the end of the quote,
      // and white words on a bright gold disc were hard to read.
      body: Color(0xFFD6B660),
      glow: Color(0xFFB9A060),
      star: _starlight,
      onSky: _lightText,
    ),
    (DayPhase.afternoon, Brightness.dark): HomeSkyColors(
      top: Color(0xFF2A5582),
      blush: Color(0xFF46668F),
      bottom: Color(0xFFD2B0A6),
      farHill: Color(0xFF3A5A88),
      land: Color(0xFF22385E),
      body: Color(0xFFD9AC58),
      glow: Color(0xFFC49A62),
      star: _starlight,
      onSky: _lightText,
    ),
    ...
    (DayPhase.lateNight, Brightness.dark): HomeSkyColors(
      // The top near black and the horizon still a faint blue, so the far
      // hills have a sky to stand against. Darker hills with no light
      // behind them measured 1.07:1 and vanished.
      top: Color(0xFF06081C),
      bottom: Color(0xFF4E5EA8),
      farHill: Color(0xFF1E2556),
      land: Color(0xFF06071C),
      body: Color(0xFFE6D6AE),
      glow: Color(0xFF9C7A58),
      star: _starlight,
      onSky: _lightText,
    ),
  };
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: colours.stops,
            stops: colours.stopsAt,
          ),
        ),
      ),
    );
...
class HomeStage extends StatefulWidget {
  ...
  // Tonight's moon. Only the two night skies draw it.
  ...
  // Her height on the hill, and the band's own height around her. Home and
  // the breathing screen both stand her here, so the number lives with the
  // hill rather than with either screen.
  static const double characterHeight = 250;
  static const double bandHeight = characterHeight + 16;

  // How far her box sits above the band's foot. The user's number, chosen by
  // eye on Home, 26 September 2026.
  static const double characterLift = 6;
...
// Made 25 September 2026. The movement is kept slow and small: a drift of a
...
class _StagePainter extends CustomPainter {
  ...
  // same distance, so the two cannot drift apart.
  static const double _glowReach = 3.0;
  ...
  // The fainter stars that come out after midnight, on the darker sky.
  static const List<(double, double, double)> _lateStars =
      <(double, double, double)>[
    (0.03, 0.26, 0.9),
    (0.10, 0.04, 0.8),
    (0.19, 0.20, 0.9),
  ...
  // Where the sun or moon sits, and how big it is, in a band of [size].
  //
  // **The sun travels across the day, left to right, and the moon across
  // the night**, as (share of the width, share of the height). Six stops
  // rather than a slide, for the reasons in `day_phase.dart`:
  //
  // | Sky | Where | Why |
  // | --- | --- | --- |
  // | Morning | Low on the left | Rising |
  // | Midday | In the middle, just above her ears | Highest in the sky. It sits behind the quote's last lines, and that is the user's call, 26 September 2026 |
  // | Afternoon | High, to the right of her ears | A step down from midday and well above the evening sun |
  // | Evening | Low on the right, bigger and warmer | Setting |
  // | Night | Up on the right | Risen |
  // | Late night | Higher, over on the left | Further across, and smaller and paler |
  static (Offset, double) bodyAt(DayPhase phase, Size size) {
    final double w = size.width;
    final double h = size.height;
    return switch (phase) {
      DayPhase.morning => (Offset(w * 0.24, h * 0.26), 28.0),
      DayPhase.midday => (Offset(w * 0.5, -40), 30.0),
      DayPhase.afternoon => (Offset(w * 0.73, h * 0.02), 28.0),
      DayPhase.evening => (Offset(w * 0.79, h * 0.34), 34.0),
      DayPhase.night => (Offset(w * 0.79, h * 0.26), 36.0),
      DayPhase.lateNight => (Offset(w * 0.2, h * 0.13), 32.0),
    };
  }
  ...
  //
  // The two on the right stand where the slope has fallen away, so their
  // feet are sunk into it, with the height given back to keep each top
  // where it was. At base 62 the one at 0.79 hung a few points above the
  // hill with sky showing under its hem.
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // The sun or moon first, at `bodyAt`. Every place is clear of every
    // peak and of her head.
    final (Offset centre, double radius) = bodyAt(phase, size);

    if (phase.hasMoon) {
      // More stars show on the darker sky. None inside the moon's glow,
      // which moves between the two nights.
      final double clear = radius * 2.2;
      paint.color = colours.star.withValues(alpha: 0.8);
      for (final (double x, double y, double r) in <(double, double, double)>[
        ..._stars,
        if (phase == DayPhase.lateNight) ..._lateStars,
      ]) {
        final Offset at = Offset(w * x, h * y);
        if ((at - centre).distance < clear) continue;
        canvas.drawCircle(at, r, paint);
      }
      _paintMoon(canvas, centre, radius);
    } else {
      // The glow: one radial gradient out to three times the disc, the same
      // way the mist is shaded. It was three flat rings stacked on each
      // other, and their edges showed as steps.
      ...
      canvas.drawCircle(
        ...
        Paint()
          ...
          ..shader = RadialGradient(
            colors: <Color>[
              colours.glow.withValues(alpha: 0.5),
              colours.glow.withValues(alpha: 0.5),
              colours.glow.withValues(alpha: 0.26),
              colours.glow.withValues(alpha: 0.1),
              colours.glow.withValues(alpha: 0),
            ],
            stops: const <double>[0, 1 / _glowReach, 0.5, 0.75, 1],
    ...
    // The mist behind her: an upright oval, strongest round her body and
    // gone before the edges, in front of the ranges and behind the nearest
    // trees and her hill. It thins out round the sun or moon along the same
  ...
  // How far the moon leans, in radians: about 20 degrees.
  static const double _moonTilt = 0.35;

  // The moon: a wide blurred halo, the lit part of the disc, and a few
  // faint seas and craters on it. Blurred rather than ringed, so the glow
  // has no steps in it.
  //
  // **Tonight's shape, not always a full moon**, from 26 September 2026 at
  // the user's request. `MoonPhase` says how much is lit and on which side,
  // turned the right way round for the hemisphere. The unlit part is still
  // there, faintly -- earthshine, the light the Earth throws back onto it,
  // is what makes a real crescent show its whole round. On the night of a
  // new moon that faint disc is all there is, which is the truth.
  void _paintMoon(Canvas canvas, Offset centre, double radius) {
    final double lit = moon.lit;

    // A thin crescent throws far less light than a full moon.
    canvas.drawCircle(
      centre,
      radius * 1.6,
      Paint()
        ..color = colours.glow.withValues(alpha: 0.08 + 0.27 * lit)
        ..maskFilter = MaskFilter.blur(BlurStyle.normal, radius * 0.9),
    );

    final Rect disc = Rect.fromCircle(center: centre, radius: radius);
    final Path round = Path()..addOval(disc);

    // Earthshine: the dark side, a faint wash of moonlight over whatever
    // sky is behind it, so it is always a shade lighter than that sky. A
    // solid colour was tried first and read as a dark ball hung in front of
    // the sky.
    canvas.drawPath(
      round,
      Paint()
        ..isAntiAlias = true
        ..color = colours.body.withValues(alpha: 0.07),
    );

    if (lit < 0.01) return;

    // The lit part: the lit half of the disc, with the terminator -- the
    // line between day and night on the moon -- drawn as half an ellipse.
    // Less than half lit, the ellipse is cut out of the half; more than
    // half, it is added to it.
    final double right = moon.litOnRight ? 1 : -1;
    final Path half = Path.combine(
      PathOperation.intersect,
      round,
      Path()
        ..addRect(Rect.fromLTRB(
          right > 0 ? centre.dx : centre.dx - radius,
          centre.dy - radius,
          right > 0 ? centre.dx + radius : centre.dx,
          centre.dy + radius,
        )),
    );
    final Path terminator = Path()
      ..addOval(Rect.fromCenter(
        center: centre,
        width: 2 * radius * (1 - 2 * lit).abs(),
        height: 2 * radius,
      ));
    final Path shape = Path.combine(
      lit < 0.5 ? PathOperation.difference : PathOperation.union,
      half,
      terminator,
    );

    // **Tilted a little to the left**, at the user's request, 26 September
    // 2026: a crescent standing bolt upright read as drawn with a ruler.
    // The real one leans too -- how far depends on the place and the hour
    // -- so a fixed lean is closer to the sky than none. Everything lit
    // turns together: the shape, its shading, the seas and the craters.
    //
    // **The lean follows the lit side, so the two halves of the month are
    // mirror images.** One lean for every night tipped the waning moons the
    // wrong way; the user asked for them turned round, 26 September 2026.
    // Lit on the left, it leans left; lit on the right, it leans right.
    canvas
      ..save()
      ..translate(centre.dx, centre.dy)
      ..rotate(moon.litOnRight ? _moonTilt : -_moonTilt)
      ..translate(-centre.dx, -centre.dy)
      ..clipPath(shape);
    canvas.drawCircle(
      centre,
      ...
      Paint()
        ..shader = RadialGradient(
          center: Alignment(0.35 * right, -0.4),
          radius: 1.1,
          colors: <Color>[
    ...
    // **Craters, kept faint**, at the user's request, 26 September 2026.
    // Each is a small dip: shaded on the side nearest the light, which is
    // the upper left, with a thin lit rim on the far side -- the way a real
    // crater catches the sun. Clipped to the lit shape, so none spills over the
    // edge. Faint enough to find, not to notice first. Only on the lit
    // part: the dark side is too dim to show any.
    final Paint shade = Paint()
      ..isAntiAlias = true
      ..color = colours.glow.withValues(alpha: 0.26);
    final Paint rim = Paint()
      ..isAntiAlias = true
      ..style = PaintingStyle.stroke
      ..color = colours.star.withValues(alpha: 0.28);
    for (final (double dx, double dy, double r) in <(double, double, double)>[
      (-0.42, 0.18, 0.11),
      (0.10, -0.52, 0.08),
      (0.52, 0.02, 0.09),
      ...
      final Offset at = centre + Offset(dx * radius, dy * radius);
      final double cr = r * radius;
      canvas.drawCircle(at, cr, shade);
      rim.strokeWidth = cr * 0.35;
      canvas.drawArc(Rect.fromCircle(center: at, radius: cr * 0.95), -0.2,
          math.pi * 0.85, false, rim);
    }
    canvas.restore();
  }
...
class _LifePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bool glowing = phase.glows;
